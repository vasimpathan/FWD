...
function pageValidate() {
   ...
   // alert(455);
	...
	else
	{
		...
		if(tr==false)
		{
			document.getElementById("errInsuHkid").innerHTML = "请输入有效的英文身份证号码";
        	flag = false;
		}
	}
    if (spouseHkid.trim() == "") {
    ...
    if (childHkid.trim() == "") {
        document.getElementById("errChildHkid").innerHTML = "請以英文輸入受益人 香港身份證號碼";
        flag = false;
    }
    else {
        var tr = IsHKID(childHkid.trim());
        if (tr == false) {
            document.getElementById("errChildHkid").innerHTML = "请输入有效的英文身份证号码";
            flag = false;
        }
    }
    if (fullname.trim() == "") {
